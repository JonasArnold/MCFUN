...
void motorSetPwmRight(int8_t value)
{
  ...
  {
    // drive forward
    // todo #09.08 complete the else-if statement
    MOTOR_RIGHT_B_GPIO();
    MOTOR_RIGHT_A_PWM();
  }
  else
...
void motorSetPwmLeft(int8_t value)
{
  ...
  if (value < 0)
  {
	// drive backward
	value = -value;             // value has to be a positive channel value!
	MOTOR_LEFT_A_PWM();       // set motor left A as timer Pin (pwm signal)
	MOTOR_LEFT_B_GPIO();        // set motor left B as GPIO Pin (high-level)
  }
  else if (value > 0)
  {
	// drive forward
...
void motorInit(void)
{
  ...
  FTM3->CONTROLS[0].CnSC = FTM_CnSC_ELSx(1) | FTM_CnSC_MSx(2);
  FTM3->CONTROLS[1].CnSC = FTM_CnSC_ELSx(1) | FTM_CnSC_MSx(2);
