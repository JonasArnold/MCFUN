...
void FTM0CH5_IRQHandler(void)
{
  ...
  // todo #08.03 implement the ISR as follows:
  // calculate the pulse or pause time
  uint16_t thisTicks = FTM0->CNT;
  uint16_t deltaTicks = 0;
  if(thisTicks < oldTicks){ // overflow happened
	  deltaTicks = (0xFFFF-oldTicks) + thisTicks;
  }
  else{  // no overflow happened
	  deltaTicks = thisTicks - oldTicks;
  }

  // determine if it is a pulse or a pause (check GPIO-Pin) and save the ticks
  if(GPIOD->PDIR & (1 << 5)) {  // pulse time was recorded
	  pulse = deltaTicks;
  }
  else {  // pause time was recorded
	  pause = deltaTicks;
...
uint8_t irGetKey(void)
{
  ...
  if (rxState == rxReceived)
  {
    ...
    {
    // todo #08.04 complete the switch statement. See "ir.h" for Key-Codes
...
void irInit(void)
{
  // todo #08.01 configure the mux of PTD5 as timer channel.
  PORTD->PCR[5] = PORT_PCR_MUX(4);  // Alt 4 for FTM0_CH5 (PTD5)

  // todo #08.02 configure the timer channel as follows:
  // Input Capture with enabled interrupt on every changing edge of the pin
  FTM0->CONTROLS[5].CnSC = FTM_CnSC_ELSx(3) | FTM_CnSC_MSx(0) | FTM_CnSC_CHIE(1);
}
#endif
