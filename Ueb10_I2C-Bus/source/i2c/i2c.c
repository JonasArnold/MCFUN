...
{
  // test
  //return i2cTest(adr); // remove me!

  // todo #10.05 implement i2cReadCmdData(...) function
  tError result;
  result = i2cStart(adr, FALSE);             // send I2C-Address and set to write
  if (result != EC_SUCCESS) return result;
...
void i2cInit(void)
{
  // todo #10.01 configure clock gating for i2c bus
  SIM->SCGC4 |= SIM_SCGC4_I2C0_MASK;

  // todo #10.02 configure MUX for SCL & SDA (and enable open drain, pull up)
  PORTE->PCR[24] = PORT_PCR_ODE(1) | PORT_PCR_PE(1) | PORT_PCR_PS(1) | PORT_PCR_MUX(5);  // Alt 5 for I2C0_SCL (PTE24)
