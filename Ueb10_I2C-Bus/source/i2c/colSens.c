...
{
  // todo #10.09 Implement the function colSensReadRawColor(...)

  tError result;
  // read all 8 bytes
  uint8_t cmd = REG_COMMAND | REG_CDATAL;  // starting at CDATAL
  result = i2cReadCmdData(I2C_COLSENS_ADR, cmd, col->data,
		  (sizeof(col->data) / sizeof(col->data[0])));
  if (result) return result;

  // calculate colors
  col->clear = (col->data[1] << 8) | col->data[0];
  col->red =  (col->data[3] << 8) | col->data[2];
  col->green =  (col->data[5] << 8) | col->data[4];
  col->blue = (col->data[7] << 8) | col->data[6];
...
tError colSensInit(void)
{
  ...
  // todo #10.06 configure timing: ATIME = 0xC0
